...
public class DaoMonitorEnvironment extends BisqEnvironment {
    ...
    public DaoMonitorEnvironment(OptionSet options) {
        this(new JOptCommandLinePropertySource(BISQ_COMMANDLINE_PROPERTY_SOURCE_NAME, checkNotNull(options)));
    }

    public DaoMonitorEnvironment(PropertySource commandLineProperties) {
        super(commandLineProperties);

        slackUrlSeedChannel = commandLineProperties.containsProperty(DaoMonitorOptionKeys.SLACK_URL_SEED_CHANNEL) ?
                (String) commandLineProperties.getProperty(DaoMonitorOptionKeys.SLACK_URL_SEED_CHANNEL) :
                "";

        slackUrlBtcChannel = commandLineProperties.containsProperty(DaoMonitorOptionKeys.SLACK_BTC_SEED_CHANNEL) ?
                (String) commandLineProperties.getProperty(DaoMonitorOptionKeys.SLACK_BTC_SEED_CHANNEL) :
                "";

        slackUrlProviderChannel = commandLineProperties.containsProperty(DaoMonitorOptionKeys.SLACK_PROVIDER_SEED_CHANNEL) ?
                (String) commandLineProperties.getProperty(DaoMonitorOptionKeys.SLACK_PROVIDER_SEED_CHANNEL) :
                "";

        port = commandLineProperties.containsProperty(DaoMonitorOptionKeys.PORT) ?
                (String) commandLineProperties.getProperty(DaoMonitorOptionKeys.PORT) :
                "80";
        ...
        return new PropertiesPropertySource(BISQ_DEFAULT_PROPERTY_SOURCE_NAME, new Properties() {
            {
                ...
                setProperty(DaoMonitorOptionKeys.SLACK_URL_SEED_CHANNEL, slackUrlSeedChannel);
                setProperty(DaoMonitorOptionKeys.SLACK_BTC_SEED_CHANNEL, slackUrlBtcChannel);
                setProperty(DaoMonitorOptionKeys.SLACK_PROVIDER_SEED_CHANNEL, slackUrlProviderChannel);
                setProperty(DaoMonitorOptionKeys.PORT, port);
